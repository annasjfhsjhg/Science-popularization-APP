...
/* ======================
   后端同步（不覆盖模拟数据）
====================== */
export function syncUserFromBackend(data) {
  if (!data) return;
...
export function syncCollectionsFromBackend(list) {
  if (!Array.isArray(list)) return;
  COLLECTIONS.length = 0;
  list.forEach(item => COLLECTIONS.push(item));
}

export function syncAchievementsFromBackend(list) {
  if (!Array.isArray(list)) return;
  ACHIEVEMENTS.length = 0;
  list.forEach(item => ACHIEVEMENTS.push(item));
}